...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        btn_load_smpl = QPushButton("🕺 加载其他 SMPL-X"); btn_load_smpl.clicked.connect(self.load_smplx_ref)
        ...
        smpl_g = QGroupBox("SMPL-X 对照设置")
        smpl_l = QVBoxLayout()
        self.chk_smpl_vis = QCheckBox("显示原始参考 (Show Ref)")
        ...
        off_l.addWidget(QLabel("Y 偏移:"))
        sp_y = QDoubleSpinBox(); sp_y.setRange(-5, 5); sp_y.setSingleStep(0.1); sp_y.setValue(1.0) # 默认错开
        ...
        # 1. Labels
        g_lbl = QGroupBox("显示标签 (Labels)"); l_g_l = QVBoxLayout()
        combo_lbl = QComboBox()
        ...
        # 2. Frames
        g_frm = QGroupBox("显示坐标系 (Frames)"); f_g_l = QVBoxLayout()
        combo_frm = QComboBox()
        ...
        # 3. Render Flags
        g_flags = QGroupBox("渲染开关 (Flags)"); fl_l = QVBoxLayout()
