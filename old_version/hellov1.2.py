...
# ============ 配置区 ============
DEFAULT_CSV_PATH = "/home/jq/38_1.csv"
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        btn_smooth = QPushButton("🌊 选区平滑 (Sav-Gol)"); btn_smooth.clicked.connect(self.apply_smooth)
    ...
    def apply_smooth(self):
        """
        改进后的平滑算法：使用 Savitzky-Golay 滤波器。
        相比简单的移动平均(rolling mean)，它能更好地保留波峰和波谷的特征，避免过度失真。
        """
        ...
        n_points = len(data_chunk)
        
        # 动态计算窗口大小。越大越平滑。
        # 目标窗口大小为 31，但不能超过数据长度，且必须是奇数。
        window_len = min(n_points, 31)
        if window_len % 2 == 0: window_len -= 1
        
        # 只有当窗口长度足够时才进行滤波
        if window_len >= 3:
            # polyorder=3 表示拟合三次曲线，适合人体运动轨迹
            smoothed_data = savgol_filter(data_chunk, window_length=window_len, polyorder=3)
            self.backend.df.iloc[s:e+1, col] = smoothed_data
            ...
            self.status_bar.showMessage(f"已平滑 (Sav-Gol, Window: {window_len})")
        else:
            self.status_bar.showMessage("选区太短，无法平滑")
    ...
    def apply_additive(self):
        ...
        self.backend.modified_frames.update(range(s, e+1)) # 确保也更新修改标记
        self.graph.update_curves([self.graph.selected_joint_idx])
        self.backend.set_frame(self.current_frame)
    ...
    def reset_original(self):
        ...
        self.backend.modified_frames.update(range(s, e+1)) # 更新标记（虽然是重置回原始，但也算一次操作）
        self.graph.update_curves([self.graph.selected_joint_idx])
        self.backend.set_frame(self.current_frame)
